  ...
  if tf.gfile.Exists(hparams_file):
    ...
    with codecs.getreader("utf-8")(tf.io.gfile.GFile(hparams_file, "rb")) as f:
      try:
        ...
        hparams = contrib.training.HParams(**hparams_values)
      except ValueError:
...
def print_current_dialogue(words1, words2, mask1, mask2):
  """This function prints out the current dialogue."""
  for w in words1:
    i, src, infer = w
    if src[0] == "<t1>":
      mk = mask1
    ...
    print(1, "src", " ".join(src))
    print(1, "res", " ".join(infer))
  ...
  for w in words2:
    ...
    print(2, "src", " ".join(src))
    print(2, "res", " ".join(infer))
    print(" ".join(src), " ".join(infer))
